...
export class HomeComponent implements OnInit {
 ...
 toggled: boolean = false;
  ...
  lcompetance=new Array<String>();
   cmp: string;
   constructor(private sharePostService:SharedPostService,private likeService:LikeService,private commentService:CommentService,private roleService : RoleService,private pubService:PubService,private taskservice:TaskService,private offerservice:OfferService,private competanceService: CompetanceService ,
  ...
  handleSelection(event) {
   ...
   checkSupervision(){
    ...
    createOffer() {
     ...
     // let pub = new Pub();
     // pub.created_at=offer.date;
     // pub.updated_at=offer.date;
     // pub.offer=offer;
     // pub.nlike=0;
     // pub.user=this.user;
   ...
   this.userService.getUserByEmail(this.username).subscribe(res => {
     ...
     // let pub = new Pub();
     // pub.created_at=task.date;
     // pub.updated_at=task.date;
     // pub.task=task;
     // pub.nlike=0;
     // pub.user=this.user;
     console.log("we start to create offer and this is the value");
   ...
   listMyTasks(){
    ...
    console.log(new Date());
     ...
     this.pubService.listPubs().subscribe(res=>{
         ...
         if(new Date().getMonth()-new Date(pub.updated_at).getMonth()==0){
           ...
           if(new Date().getDay()-new Date(pub.updated_at).getDay()==0){
             ...
             if(new Date().getHours()-new Date(pub.updated_at).getHours()==0){
               ...
               //pub.updated_at= moment(new Date().getHours()-new Date(pub.updated_at).getHours()).toDate();
            ...
            // pub.updated_at= moment(new Date().getDay()-new Date(pub.updated_at).getDay()).toDate();
          ...
          //pub.updated_at= moment(new Date().getMonth()-new Date(pub.updated_at).getMonth()).toDate();
         }
       ...
       //   if(moment(element.updated_at).hour()-moment(new Date()).hour()>0){
       //  let datee= moment(element.updated_at).hour()-moment(new Date()).hour();
       //   console.log(datee);
       //   }        
       //   if(moment(element.updated_at).minutes()-moment(new Date()).minutes()>0){
       //     let datee= moment(element.updated_at).minutes()-moment(new Date()).minutes();
       //   console.log(datee);
       //   }
       })
